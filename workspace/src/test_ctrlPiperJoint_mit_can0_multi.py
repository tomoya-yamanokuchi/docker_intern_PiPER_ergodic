...
from impedance_control.mit import send_mit
from impedance_control.piper import enable, get_pos, stop
...
if __name__ == "__main__":
    ...
    print("INFO: Current joints:", get_pos(piper, False))
    ...
    base = list(get_pos(piper, False))

    q0 = {joint: base[joint - 1] for joint in MIT_JOINTS}
    ...
    send_mit(interface, q0, MIT_JOINTS, VEL_REF, KP, KD, TORQUE_FF)
    ...
    try:
        for cycle in range(NUM_CYCLES):
            ...
            for targets in targets_list:
                send_mit(interface, targets, MIT_JOINTS, VEL_REF, KP, KD, TORQUE_FF)

                time.sleep(HOLD_TIME)

                current = get_pos(piper, False)
