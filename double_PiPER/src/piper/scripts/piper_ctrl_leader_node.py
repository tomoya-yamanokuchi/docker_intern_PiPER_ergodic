...
def check_ros_leader():
    try:
        rosnode.rosnode_ping('rosout', max_count=1, verbose=False)
        rospy.loginfo("ROS Leader is running.")
    except rosnode.ROSNodeIOException:
        rospy.logerr("ROS Leader is not running.")
        raise RuntimeError("ROS Leader is not running.")
...
class C_PiperRosNode():
    ...
    def __init__(self) -> None:
        ...
        self.joint_grip_pub = rospy.Publisher('joint_grip_pose', JointState, queue_size=1)#develop
        # service
        ...
        self.reset_service = rospy.Service('go_home', Trigger, self.handle_go_home_service)
    ...
    def Pubilsh(self):
        ...
        while not rospy.is_shutdown():
            ...
            self.PublishArmJointAndGripperCmd() ##develop
            rate.sleep()
    ...
    ###develop (Slave側のCTRLに都合が良いコマンドを公開) 
    def PublishArmJointAndGripperCmd(self):
        joint_state_sdk = self.piper.GetArmJointMsgs().joint_state
        ...
        msg.position = [
            joint_state_sdk.joint_1/1000 * 0.017444,
            joint_state_sdk.joint_2/1000 * 0.017444,
            joint_state_sdk.joint_3/1000 * 0.017444,
            joint_state_sdk.joint_4/1000 * 0.017444,
            joint_state_sdk.joint_5/1000 * 0.017444,
            joint_state_sdk.joint_6/1000 * 0.017444,
            gripper_state.grippers_angle / 1000000
        ]
        self.joint_grip_pub.publish(msg)
    ...
    def handle_go_home_service(self, req):
        """
        ROSサービス: アームをHome Positionに移動させる安全版
        """
        response = TriggerResponse()
        ...
        def move_home():
            try:
                ...
                print("Teach status:", self.piper.GetArmStatus().arm_status.ctrl_mode)
                print("Enable flag:", self.piper.GetArmLowSpdInfoMsgs().motor_1.foc_status.driver_enable_status)

                # 外部からのジョイント制御を一時停止
                self.block_ctrl_flag = True
                rospy.sleep(0.05)

                # Teachモード終了
                self.piper.MotionCtrl_1(0x00, 0x00, 0x02)  # Teach終了
                rospy.sleep(0.1)

                # MoveJモード設定（低速で安全に）
                self.piper.MotionCtrl_2(0x01, 0x01, 20, 0)
                rospy.sleep(0.1)

                # 全関節をHome位置に移動（Homeが0の場合）
                self.piper.JointCtrl(0, 0, 0, 0, 0, 0)
                rospy.sleep(2.0)  # モーション完了まで待つ

                # 末端グリッパーも初期化
                if self.gripper_exist:
                    self.piper.GripperCtrl(0, 1000, 0x01, 0)

                rospy.loginfo("Arm reached home position successfully.")
        ...
        # サービス呼び出しでは非同期に移動
        t = threading.Thread(target=move_home)
        t.start()

        response.success = True
        response.message = "Home motion started."
        return response
